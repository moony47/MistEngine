...
public abstract class ManagedScript {
	...
	private static List<string> s_CachedScriptTypeNames = new();
	private static bool s_TypesCached = false;
	...
	[UnmanagedCallersOnly]
	public static void DiscoverManagedScriptTypes (bool refresh) {
		try {
			if (s_TypesCached && !refresh) {
				Console.WriteLine($"[ManagedRuntime] ManagedScript types already cached ({s_CachedScriptTypeNames.Count} types)");
				return;
			}

			s_CachedScriptTypeNames.Clear();
			Type baseType = typeof(ManagedScript);
			...
			foreach (var type in derivedTypes) {
				if (type.FullName != null) {
					s_CachedScriptTypeNames.Add(type.FullName);
				}
			}

			s_TypesCached = true;
			Console.WriteLine($"[ManagedRuntime] Discovered {s_CachedScriptTypeNames.Count} ManagedScript types");
			foreach (string typeName in s_CachedScriptTypeNames) {
				Console.WriteLine($"  - {typeName}");
			}
		} catch (Exception ex) {
			Console.WriteLine($"[ManagedRuntime] Error in DiscoverManagedScriptTypes: {ex}");
		}
	}

	[UnmanagedCallersOnly]
	public static int GetManagedScriptTypeCount () {
		try {
			if (!s_TypesCached) {
				return 0;
			}
			return s_CachedScriptTypeNames.Count;
		} catch (Exception ex) {
			Console.WriteLine($"[ManagedRuntime] Error in GetManagedScriptTypeCount: {ex}");
			return 0;
		}
	}

	[UnmanagedCallersOnly]
	public static unsafe char* GetManagedScriptTypeAt (int index) {
		try {
			if (!s_TypesCached || index < 0 || index >= s_CachedScriptTypeNames.Count) {
				return null;
			}

			string typeName = s_CachedScriptTypeNames[index];
			int size = typeName.Length + 1;
			char* buffer = (char*) Marshal.AllocCoTaskMem(size * sizeof(char));

			fixed (char* src = typeName) {
				Buffer.MemoryCopy(src, buffer, size * sizeof(char), typeName.Length * sizeof(char));
			}
			buffer[typeName.Length] = '\0';

			return buffer;
		} catch (Exception ex) {
			Console.WriteLine($"[ManagedRuntime] Error in GetManagedScriptTypeAt: {ex}");
			return null;
		}
	}

	[UnmanagedCallersOnly]
	public static unsafe void FreeManagedScriptTypeName (char* ptr) {
		try {
			if (ptr != null) {
				Marshal.FreeCoTaskMem((IntPtr) ptr);
			}
		} catch (Exception ex) {
			Console.WriteLine($"[ManagedRuntime] Error in FreeManagedScriptTypeName: {ex}");
		}
	}
