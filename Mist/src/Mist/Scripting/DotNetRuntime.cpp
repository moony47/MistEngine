...
#include "Mist/Scripting/ScriptingCallbacks.h"
...
namespace Mist {
	...
	bool DotNetRuntime::InitializeRuntime() {
		...
		int size_needed = MultiByteToWideChar(CP_UTF8, 0, &assembly_path.string()[0], (int) assembly_path.string().size(), NULL, 0);
		std::wstring wide_assembly_path(size_needed, 0);
		MultiByteToWideChar(CP_UTF8, 0, &assembly_path.string()[0], (int) assembly_path.string().size(), &wide_assembly_path[0], size_needed);
		...
		on_button_fn on_key_pressed = nullptr;
		on_button_fn on_key_released = nullptr;

		result = m_LoadAssemblyFunc(wide_assembly_path.c_str(), L"Mist.Scripting.ManagedScript, ScriptEngine",
									L"OnKeyPressedNative", (const wchar_t*) (char*) -1, nullptr, (void**) &on_key_pressed);
		if(result != 0)
			MIST_ERROR("Failed to get ManagedScript.OnKeyPressedNative function (error code: {0})", result);

		result = m_LoadAssemblyFunc(wide_assembly_path.c_str(), L"Mist.Scripting.ManagedScript, ScriptEngine",
									L"OnKeyReleasedNative", (const wchar_t*) (char*) -1, nullptr, (void**) &on_key_released);
		if(result != 0)
			MIST_ERROR("Failed to get ManagedScript.OnKeyReleasedNative function (error code: {0})", result);


		on_button_fn on_mouse_pressed = nullptr;
		on_button_fn on_mouse_released = nullptr;
		on_move_fn on_mouse_moved = nullptr;
		on_move_fn on_mouse_scrolled = nullptr;

		result = m_LoadAssemblyFunc(wide_assembly_path.c_str(), L"Mist.Scripting.ManagedScript, ScriptEngine",
									L"OnMousePressedNative", (const wchar_t*) (char*) -1, nullptr, (void**) &on_mouse_pressed);
		if(result != 0)
			MIST_ERROR("Failed to get ManagedScript.OnMousePressedNative function (error code: {0})", result);

		result = m_LoadAssemblyFunc(wide_assembly_path.c_str(), L"Mist.Scripting.ManagedScript, ScriptEngine",
									L"OnMouseReleasedNative", (const wchar_t*) (char*) -1, nullptr, (void**) &on_mouse_released);
		if(result != 0)
			MIST_ERROR("Failed to get ManagedScript.OnMouseReleasedNative function (error code: {0})", result);

		result = m_LoadAssemblyFunc(wide_assembly_path.c_str(), L"Mist.Scripting.ManagedScript, ScriptEngine",
									L"OnMouseMovedNative", (const wchar_t*) (char*) -1, nullptr, (void**) &on_mouse_moved);
		if(result != 0)
			MIST_ERROR("Failed to get ManagedScript.OnMouseMovedNative function (error code: {0})", result);

		result = m_LoadAssemblyFunc(wide_assembly_path.c_str(), L"Mist.Scripting.ManagedScript, ScriptEngine",
									L"OnMouseScrolledNative", (const wchar_t*) (char*) -1, nullptr, (void**) &on_mouse_scrolled);
		if(result != 0)
		...
		ManagedScript::s_OnDestroyFunc = on_destroy;

		ManagedScript::s_OnKeyPressedFunc = on_key_pressed;
		ManagedScript::s_OnKeyReleasedFunc = on_key_released;
		ManagedScript::s_OnMousePressedFunc = on_mouse_pressed;
		ManagedScript::s_OnMouseReleasedFunc = on_mouse_released;
		ManagedScript::s_OnMouseMovedFunc = on_mouse_moved;
		ManagedScript::s_OnMouseScrolledFunc = on_mouse_scrolled;
	...
	bool DotNetRuntime::RegisterCallbacks() {
		...
		success &= m_RegisterCallback(L"GetID", (void*) &ScriptingCallbacks::GetID);
		success &= m_RegisterCallback(L"GetName", (void*) &ScriptingCallbacks::GetName);

		success &= m_RegisterCallback(L"IsKeyPressed", (void*) &ScriptingCallbacks::IsKeyPressed);
		success &= m_RegisterCallback(L"IsMouseButtonPressed", (void*) &ScriptingCallbacks::IsMouseButtonPressed);
		success &= m_RegisterCallback(L"GetMousePosition", (void*) &ScriptingCallbacks::GetMousePosition);
	...
	void DotNetRuntime::Shutdown() {
		...
		ManagedScript::s_OnKeyPressedFunc = nullptr;
		ManagedScript::s_OnKeyReleasedFunc = nullptr;
		ManagedScript::s_OnMousePressedFunc = nullptr;
		ManagedScript::s_OnMouseReleasedFunc = nullptr;
		ManagedScript::s_OnMouseMovedFunc = nullptr;
		ManagedScript::s_OnMouseScrolledFunc = nullptr;
