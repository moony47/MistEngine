...
protected:
    ...
    virtual ShaderProgramSource ParseShader(const std::string& vertShaderPath,
                                            const std::string& fragShaderPath) const = 0;
    virtual std::expected<uint32_t, std::string> CompileShader(uint32_t type, const std::string& source) const = 0;
    virtual std::expected<uint32_t, std::string> CreateShader(const std::string& vertexShader,
