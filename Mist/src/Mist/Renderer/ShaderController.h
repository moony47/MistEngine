...
public:
    ...
    virtual Shader* CreateShader(const std::string& vertShaderPath, const std::string& fragShaderPath) = 0;
    virtual Texture2D* CreateTexture(const std::string& texturePath) = 0;

    virtual void DeregisterShader(uint32_t shaderID) = 0;
	virtual void DeregisterTexture(uint32_t textureID) = 0;

    inline static ShaderController* GetInstance() { return s_Instance; }
